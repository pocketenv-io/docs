...
```bash
pocketenv create my-first-sandbox
```

By default this uses the `cloudflare` provider and the `openclaw` base image — a minimal Linux environment with common dev tools pre-installed.
...
```bash
pocketenv stop my-first-sandbox    # Stop the sandbox (keeps state)
